...
Что делает:
  ...
  2. Копирует Assets/* → Frontend/Core/assets/
...
# ── Форсим UTF-8 для stdout/stderr (важно для Windows CI) ──
...
# ============================================================
#  Логирование (безопасное для любых кодировок)
# ============================================================
...
def log(msg: str) -> None:
    try:
        ...
        print(f"[build_frontend] {msg.encode('ascii', 'replace').decode('ascii')}",
              flush=True)
...
def warn(msg: str) -> None:
    try:
        ...
        print(f"[build_frontend][WARN] {msg.encode('ascii', 'replace').decode('ascii')}",
              flush=True)
...
def die(msg: str) -> None:
    try:
        ...
        print(f"[build_frontend][ERROR] {msg.encode('ascii', 'replace').decode('ascii')}",
...
def find_flutter() -> str:
    ...
    if flutter_root:
        ...
        if candidate.is_file():
            log(f"Flutter from FLUTTER_ROOT: {candidate}")
    ...
    for name in names:
        ...
        if found:
            log(f"Flutter found in PATH: {found}")
            return found
    ...
    for d in path_dirs:
        ...
        for name in names:
            ...
            if candidate.is_file():
                log(f"Flutter found manually: {candidate}")
                return str(candidate)
    ...
    for r in common_roots:
        ...
        if candidate.is_file():
            log(f"Flutter found in standard location: {candidate}")
            return str(candidate)

    die(
        "flutter not found.\n"
        "  1) Install Flutter: https://docs.flutter.dev/get-started/install\n"
        "  2) Add <flutter_dir>/bin to PATH and restart terminal\n"
        "  3) Or set FLUTTER_ROOT=<flutter_dir>"
    )
    return ""
...
def run_flutter(flutter: str, args: list, cwd: Path) -> None:
    log(f"$ {' '.join([Path(flutter).name] + args)}  (cwd: {cwd})")
    ...
    try:
        ...
        die(f"failed to launch {flutter}")
        return
    except OSError as e:
        die(f"launch error {flutter}: {e}")
        return
...
def copy_assets_to_dart() -> None:
    ...
    if count == 0:
        warn(f"{ASSETS} is empty - no images copied")
    else:
        log(f"Copied assets: {count}")
...
def build_dart(flutter: str) -> None:
    if not DART_CORE.exists():
        die(f"Dart project not found: {DART_CORE}")
    if not (DART_CORE / "pubspec.yaml").exists():
        die(f"No pubspec.yaml in {DART_CORE}")
    ...
    if not built.exists():
        die(f"After build, {built} not found")

    if OUTPUT.exists():
        shutil.rmtree(OUTPUT)
    OUTPUT.mkdir(parents=True)

    log(f"Copying {built} -> {OUTPUT}")
...
def copy_api(platform_name: str) -> None:
    if platform_name not in API_FILES:
        die(
            ...
            f"Available: {', '.join(API_FILES)}"
        )

    src = API_DIR / API_FILES[platform_name]
    if not src.exists():
        die(f"Api file not found: {src}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build LaLune Dart frontend into Frontend/output/",
    )
    parser.add_argument(
        "--platform",
        required=True,
        choices=list(API_FILES.keys()),
        help="Target platform (selects which Api/*.js to copy)",
    )
