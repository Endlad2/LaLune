// iOS API + UI для LaLune
// Полная логика: мост к Swift + рендер UI
...
if (typeof window._lalune_loaded === 'undefined') {
    ...
    window._iosCallbacks = {};

    window._iosCallback = function(id, result) {
        if (window._iosCallbacks[id]) {
            window._iosCallbacks[id](result);
            delete window._iosCallbacks[id];
        }
    };
    ...
    function callNative(method, params) {
        return new Promise(function(resolve) {
            ...
            window._iosCallbacks[callbackId] = resolve;
            
            window.webkit.messageHandlers.lalune.postMessage({
                method: method,
    ...
    function getApi() {
        return {
            GetConfigsJson: function() { return callNative('getConfigs'); },
            GetSettingsJson: function() { return callNative('getSettings'); },
            GetLogsJson: function() { return callNative('getLogs'); },
            GetStatusJson: function() { return callNative('getStatus'); },
            SaveConfig: function(link) { return callNative('saveConfig', { link: link }); },
            DeleteConfig: function(id) { return callNative('deleteConfig', { id: id }); },
            SaveSettings: function(json) { return callNative('saveSettings', { settings: json }); },
            Connect: function(id) { return callNative('connect', { configId: id }); },
            Disconnect: function() { return callNative('disconnect'); },
            ClearLogs: function() { return callNative('clearLogs'); },
            CheckUpdate: function() { return callNative('checkUpdate'); },
            UpdateCoreAndWait: function() { return callNative('updateCoreAndWait'); }
        };
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        console.log('LaLune iOS UI загружен');
        createStars();
        loadConfigs();
        startPolling();
    ...
    function startPolling() {
        setInterval(function() {
            getApi().GetStatusJson().then(function(result) {
                try {
    ...
    function loadConfigs() {
        getApi().GetConfigsJson().then(function(result) {
            try {
    ...
    function deleteConfig(id) {
        ...
        getApi().DeleteConfig(id).then(function(result) {
            if (result === 'true' || result === true) {
                showToast('Конфиг удален');
                if (selectedConfigId === id) {
                    selectedConfigId = null;
                    currentSettings = { peer: '', vkHashes: '', password: '', workersPerHash: 9 };
                }
                loadConfigs();
            } else {
    ...
    function saveConfig() {
        ...
        getApi().SaveConfig(link).then(function(result) {
            if (result === 'true' || result === true) {
    ...
    function connectWithUpdateCheck() {
        ...
        getApi().CheckUpdate().then(function(result) {
            try {
    ...
    function connect() {
        ...
        getApi().SaveSettings(JSON.stringify(settings)).then(function(saved) {
            if (saved === 'true' || saved === true) {
                if (selectedConfigId !== null) {
                    getApi().Connect(selectedConfigId).then(function(result) {
                        if (result === 'true' || result === true) {
    ...
    function disconnect() {
        getApi().Disconnect().then(function() {
            showToast('Отключено');
        }).catch(function() {});
    }
    ...
    function loadLogs() {
        getApi().GetLogsJson().then(function(result) {
            try {
    ...
    function clearLogs() {
        getApi().ClearLogs().then(function() {
            const content = document.getElementById('logsContent');
                ...
                <div class="settings-group">
                    ...
                    <div class="group-body open">
                        <div class="settings-row">
                            <label>Peer</label>
                            <input type="text" id="settingPeer" value="${currentSettings.peer || ''}">
                        </div>
                        <div class="settings-row">
                            <label>VK Hashes</label>
                            <input type="text" id="settingVkHashes" value="${currentSettings.vkHashes || ''}">
                        </div>
                        <div class="settings-row">
                            <label>Password</label>
                            <input type="password" id="settingPassword" value="${currentSettings.password || ''}">
                        </div>
                        <div class="settings-row">
    ...
    function saveSettings() {
        ...
        getApi().SaveSettings(JSON.stringify(settings)).then(function(result) {
            if (result === 'true' || result === true) {
    ...
    function renderInfoTab(container) {
        ...
        infoBlock.innerHTML = `
            ...
            <div class="info-sub">iOS Client v0.5.0</div>
    ...
    function showUpdateBanner(version) {
        ...
        if (!banner) return;
        banner.innerHTML = '<span>Доступна новая версия: ' + version + '</span>';
    ...
    console.log('LaLune iOS API загружен');
}
