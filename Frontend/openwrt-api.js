...
function toggleConnect() {
    if (isConnected) {
        apiPost('/api/disconnect').then(function(data) {
            if (data.success) {
                setConnected(false);
            }
        });
    ...
    apiPost('/api/connect', { config_id: selectedConfigId }).then(function(data) {
        if (data.success) {
            setConnected(true);
...
function saveConfig() {
    ...
    apiPost('/api/configs', { link: link }).then(function(data) {
        closeAddModal();
        if (data.success) {
...
function deleteConfig(id) {
    ...
    apiDelete('/api/configs/' + id).then(function(data) {
        if (data.success) {
            if (selectedConfigId === id) selectedConfigId = null;
            showToast('Конфиг удалён');
            loadConfigsFromServer();
        }
    });
}

function loadConfigsFromServer() {
    apiGet('/api/configs').then(function(data) {
        if (data.success) {
...
function loadLogs() {
    apiGet('/api/logs').then(function(data) {
        if (data.success) {
...
function clearLogs() {
    apiDelete('/api/logs').then(function(data) {
        if (data.success) {
            var container = document.getElementById('logsContainer');
...
function saveSettings() {
    ...
    apiPost('/api/settings', { settings: settings }).then(function(data) {
        if (data.success) {
