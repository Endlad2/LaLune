// Android API для LaLune
// UI-логика скопирована из desktop-api.js
// Мост: window.go.main.App (Wails) → window.lalune (AndroidBridge)
...
if (typeof window._lalune_loaded === 'undefined') {
    ...
    function bridgeCall(method, ...args) {
        return new Promise(function(resolve) {
            try {
                if (typeof window.lalune === 'undefined' || typeof window.lalune[method] !== 'function') {
                    console.error('[android.js] Метод lalune.' + method + ' недоступен');
                    resolve(null);
                    return;
                }
                const result = window.lalune[method].apply(window.lalune, args);
                resolve(result);
            } catch (e) {
                console.error('[android.js] Ошибка вызова lalune.' + method + ':', e);
                resolve(null);
            }
        });
    }
    ...
    const AndroidApi = {
        ...
        DeleteConfig:    (id)   => bridgeCall('deleteConfig', Number(id)),
        GetSettingsJson: ()     => bridgeCall('getSettings'),
        SaveSettings:    (json) => bridgeCall('saveSettings', json),
        GetLogsJson:     ()     => bridgeCall('getLogs'),
        ClearLogs:       ()     => bridgeCall('clearLogs'),
        GetStatusJson:   ()     => bridgeCall('getStatus'),
        Connect:         (id)   => bridgeCall('connect', Number(id)),
        Disconnect:      ()     => bridgeCall('disconnect'),
    ...
    document.addEventListener('DOMContentLoaded', function() {
        console.log('LaLune Android UI загружен');
        createStars();
        loadConfigs();
        startPolling();
