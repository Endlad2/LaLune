...
#
# LaLune OpenWRT — установщик / обновлятор для OpenWRT с apk (>= 25.x).
#
# Фронтенд вкомпилирован в бинарник (go:embed web), поэтому сюда его
# отдельно качать не надо.
#
# Что делает:
#   1) apk update + ставит curl, ca-bundle, kmod-tun
...
#
# Запуск:
#   wget -qO- https://raw.githubusercontent.com/Endlad2/LaLune/main/OpenWRT/lalune-owrt-install-apk.sh | sh
...
# ============================================================
#  0. Проверка пакетного менеджера
# ============================================================

if ! command -v apk >/dev/null 2>&1; then
    die "apk не найден. Похоже, у вас старая OpenWRT с opkg — используйте lalune-owrt-install-opkg.sh"
fi
...
# apk update тянет индексы, чтобы увидеть свежие версии пакетов.
# Если падает (нет сети / уже обновлено) — не критично, просто warn.
log "apk update..."
apk update || warn "apk update завершился с ошибкой — продолжаю"

log "Устанавливаю зависимости (curl ca-bundle kmod-tun)..."
apk add curl ca-bundle kmod-tun || die "не удалось установить зависимости"
